...
const InsertChapter = () => {
    ...
    const { insertChapter } = chapterApi();
    const {afterChapterNumber} = location.state;
    const [isLoading, setIsLoading] = useState(false);
    const [formData, setFormData] = useState({
        title: '',
        content: '',
        afterChapterNumber: afterChapterNumber || ''
    });
    ...
    const handleSubmit = async (e) => {
        ...
        if(!afterChapterNumber){
            toast.error('Xảy ra lỗi, vui lòng thử lại', { autoClose: 1000 });
            return;
        }

        setIsLoading(true);
        try {
            const response = await insertChapter(storyId, formData);
